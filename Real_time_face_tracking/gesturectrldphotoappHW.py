...
# Webcam setup
cap = cv2.VideoCapture(1) # Changed to 0 (default camera). Change back to 1 if using an external webcam.
...
while True:
    ...
    # Apply the current filter FIRST
    filtered_img = apply_filter(img, filters[current_filter])
    
    # If grayscale, convert back to BGR so we can draw colored landmarks on top of it
    if filters[current_filter] == 'GRAYSCALE':
        filtered_img = cv2.cvtColor(filtered_img, cv2.COLOR_GRAY2BGR)

    # Keep a "clean" copy of the filtered image to save (without landmarks drawn on it)
    clean_save_img = filtered_img.copy()

    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            # Draw landmarks on the display image
            mp_draw.draw_landmarks(filtered_img, hand_landmarks, mp_hands.HAND_CONNECTIONS)
            ...
            # Draw circles for landmarks on the display image
            cv2.circle(filtered_img, (thumb_x, thumb_y), 10, (255, 0, 0), cv2.FILLED)
            cv2.circle(filtered_img, (index_x, index_y), 10, (0, 255, 0), cv2.FILLED)
            cv2.circle(filtered_img, (middle_x, middle_y), 10, (0, 0, 255), cv2.FILLED)
            cv2.circle(filtered_img, (ring_x, ring_y), 10, (255, 255, 0), cv2.FILLED)
            cv2.circle(filtered_img, (pinky_x, pinky_y), 10, (255, 0, 255), cv2.FILLED)
            ...
            if abs(thumb_x - index_x) < 30 and abs(thumb_y - index_y) < 30:
                if current_time - last_action_time > debounce_time:
                    ...
                    cv2.putText(filtered_img, "Picture Captured!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    last_action_time = current_time
                    
                    # Save the clean image (filtered, but NO landmarks)
                    filename = f"picture_{int(time.time())}.jpg"
                    cv2.imwrite(filename, clean_save_img)
